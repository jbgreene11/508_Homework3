...
#parking meters for Jay
parking_meters <- 
  st_read('C:/Users/green/OneDrive/Desktop/chicagoparkingmeters.geojson') %>% 
  dplyr::select(geometry) %>% 
  st_transform(st_crs(fishnet)) %>%
  mutate(Legend = "parking_meters")
...
vars_net <- 
  rbind(abandonCars,streetLightsOut,abandonBuildings,
        liquorRetail, graffiti, sanitation, parking, parking_meters, garbage_cart, supermarkets, car_shop, car_repair ) %>%
...
test <- data.frame(st_coordinates(parking_meters))
test2 <- data.frame(st_coordinates(parking_meters)) %>%
  na.omit()
...
#ADD BASELINE REGRESSION BACK INTO HERE
```
...
reg.vars <- c("Abandoned_Buildings.nn", "Abandoned_Cars.nn", "Graffiti.nn", 
              "Liquor_Retail.nn", "Street_Lights_Out.nn", "Sanitation.nn", "Parking_meters.nn", "Garbage_cart.nn", 
